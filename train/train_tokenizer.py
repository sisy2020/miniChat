...
#<think>用户让我回答***</think>*****（answer）*****
# 特殊tokens（15个）减少模型学习负担
SPECIAL_TOKENS = [
    "<|endoftext|>",   # 0 文章结束
    "<|im_start|>",    # 1
    "<|im_end|>",      # 2 对话结束
    "<think>",         # 3 学习思考范式，思维链，类似<|assistant|>，慢思考，把回答拉的很长，有演算推导的过程，把更多时间花在输出token上
    "</think>",        # 4 think结束
    "<pad>",           # 5
    "<unk>",           # 6
    "<|system|>",      # 7
...
def train_tokenizer(data_path, tokenizer_dir, vocab_size, special_tokens, max_lines=None):
    ...
    tokenizer.train_from_iterator(texts, trainer=trainer)#训练
