...
sft distill做了思维链的蒸馏，快思考->慢思考，在此基础上进行
核心逻辑：
1. 格式检查：<think>\n...\n</think>\n... (严格匹配)，sft后可能出现一个<think>两个</think>的情况
2. 格式错误 → reward=0
...
from model.model_miniChat_pro import miniChatForCausalLM
...
def calculate_rewards(raw_prompts, responses, args):
    ...
    # solve_all，solve_none，avg3=pass3 没有优势值
...
def compute_logprobs(model, outputs, completion_len):
    """
    计算 completion 部分每个 token 的 log probability，用于计算KL散度
    x=log(a/b)=log(a)-log(b)  a:πΘ  b:πref
    KL=e^x-x-1
    Args:
    ...
    #    去掉最后一个位置（因为它没有 next token，不算eos的概率）
...
def create_eos_mask(completion_ids, eos_token_id):
    ...
    # bs=16,rollout=4->(64*seq_len)，但每个prompt长度不同，model.generate(16,4)
    #这些样本如何做推理呢？有的先推理结束（早早到eos了），有的还没结束推理，先推完的怎么办，它得跟着没结束的继续推啊（因为并行）
    #早推理完的就接着往下生成eos，这样生成的也是一个矩阵，则有些resp会跟着一堆eos
    #所以要让loss不算后补充的eos token
...
def train_epoch(epoch, loader, iters, model, ref_model, optimizer, tokenizer, autocast_ctx, args,
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        # 1. 生成（左 padding），保证让prompt的最后一个token去接着预测，而不是padding去预测
        # 做推理，若不左padding，则padding token去预测下一个token，预测中间就加了padding token了，打乱了预测。
        ...
        # 生成模型推理结果
        # num_return_sequences 一个 rompt输出多少条回复
        with torch.no_grad():
        ...
        # 截取回答resp部分，只算completion log probs
        completion_ids = outputs[:, input_len:]
        
        # 2. 计算 log probs log(π)
        with autocast_ctx:
        ...
        # 方差限制在[-10,10]，即clip优势值
        advantages = torch.clamp((rewards - mean_r) / (std_r + 1e-4), -10, 10)
        # 优势值=（分数-均值）/方差
        advantages = (advantages - advantages.mean()) / (advantages.std() + 1e-8)
        
        # 5. GRPO loss on-policy
        # eos mask，mask掉rollout阶段短回答的eos
        mask = create_eos_mask(completion_ids, tokenizer.eos_token_id)
        # KL=ex-x-1 x=log(ref/policy)
        kl = torch.exp(ref_logps - policy_logps) - (ref_logps - policy_logps) - 1
        # -[ (πΘ/πold)*优势值 ] - βKL，因为πΘ=πold，所以不需要裁剪πΘ/πold的值了
        # πold参数不更新，所以detach()
        ...
        if (step + 1) % args.accumulation_steps == 0:
            # 计算梯度二范数（在 clip 之前），相当于裁剪梯度，防止梯度爆炸
        ...
        # 8. 保存 rollout 数据，单机多卡，只有在主进程时才保存
        if is_main_process():
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="miniChat GRPO Training")
    ...
    parser.add_argument("--judge_api_key", type=str, default='sk-8cecade314c443979af27f12e786c0d0')
    ...
    parser.add_argument("--swanlab_project", type=str, default="miniChat-GRPO")
    ...
    def load_model(trainable=True):
        m = miniChatForCausalLM(lm_config)
