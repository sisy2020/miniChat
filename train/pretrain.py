...
for epoch in range(num_epochs):
    for batch in dataloader:
        ...
        loss.backward()
        optimizer.step()
scale law:Loss∼f(model size,token count)参数规模,token 数量
"""
...
from model.config import miniChatConfig
from model.model_studentx import miniChatForCausalLM
...
def train_epoch(epoch, loader, iters, start_step=0, swanlab=None, total_steps=None, warmup_steps=None, full_save_dir=None):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        #forward自动转化为bf16
        with autocast_ctx:#with下的语句都在半精度进行
            res = model(input_ids, labels=labels)
            loss = res.loss / args.accumulation_steps
            #梯度累积，几个step更新一次参数
            #如batch_size=128等价于batch_size=32,accumulation_steps=4

        #混合精度
        #和autocast_ctx配套使用，防止下溢出，当数字小到当前精度无法表示时，会先扩大数字增加精度再计算
        scaler.scale(loss).backward()

        if (step + 1) % args.accumulation_steps == 0:
            scaler.unscale_(optimizer)#optimizer.step()
            #梯度裁剪，防止梯度剧烈波动，如果有的梯度norm(模长)大于0.1，这个梯度就不要了，step白做
            torch.nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip)
            scaler.step(optimizer)#optimizer.step()
            scaler.update()
            #默认梯度累积，所以每个step要optimizer.zero_grad()，防止梯度一直累积
        ...
        #每save_interval个step保存一次模型快照（保存一次模型，后面可以直接接着这里，完全无损的往下训）
        if (global_step % args.save_interval == 0 or step == iters - 1) and is_main_process():
            ...
            #不能只保存模型参数，会不知道模型训到哪里了，二阶梯度、动量梯度也要保存
            torch.save({
        ...
        if args.eval_bench == 1 and tokenizer is not None and global_step % args.eval_interval == 0 and is_main_process():
            ...
            c3_path = '/root/autodl-tmp/studentX/benchmark/clue_c3_eval_500.jsonl'
            xcopa_path ='/root/autodl-tmp/studentX/benchmark/xcopa_zh_merged.jsonl' 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="/root/autodl-tmp/studentX/miniChatPRO_pretrain_512_final.bin", help="预处理后的.bin文件路径")
    parser.add_argument('--from_weight', default='none', type=str, help="基于哪个权重训练，为none则从头开始")
    parser.add_argument('--from_resume', default=0, type=int, choices=[0, 1], help="是否自动检测&续训（0=否，1=是）")
    parser.add_argument("--use_swanlab", type=int, default=1, choices=[0, 1], help="是否使用swanlab（0=否，1=是）")
    parser.add_argument("--swanlab_project", type=str, default="studentX-Pretrain", help="swanlab项目名")
    ...
    # ========== 2. 配置目录、模型参数、检查ckp ==========
    lm_config = miniChatConfig(hidden_size=args.hidden_size, num_hidden_layers=args.num_hidden_layers)
    
    # 生成 run_name（用于后续创建子目录），区分不同参数的实验，不同参数实验在不同文件夹下
    ...
    if args.use_swanlab and is_main_process():  # [DDP] 仅主进程上报；without_ddp 无 is_main_process()
        import swanlab
        swanlab.login(api_key="jhr1kMUdxcUWhIVK4lPga")
    ...
    if args.from_weight != 'none' and os.path.exists(args.from_weight):
        ...
        #如果是sft，则可以直接从pretrain的W开始训
        model = miniChatForCausalLM.from_pretrained(args.from_weight)
    else:
        Logger(f'Creating new model: hidden_size={args.hidden_size}, num_layers={args.num_hidden_layers}')
        #加载模型,随机初始化W
        model = miniChatForCausalLM(lm_config)
    
    model = model.to(args.device)
    Logger(f'Model parameters: {sum(p.numel() for p in model.parameters())/1e6:.2f}M')
    
    # 加载 tokenizer（用于 benchmark 评测）
    #pretrain不需要训练时分词，而是事先分好词，因为要保证满token(512)
    # 训练开始前先评测一次
    if args.eval_bench == 1:
        from transformers import AutoTokenizer
        tokenizer = AutoTokenizer.from_pretrained('/root/autodl-tmp/studentX/tokenizer_15k')
