...
def eval_multiple_choice(model, tokenizer, context, choices, label_idx, max_length=512):
    """
    多选题评测：计算一条样本的每个选项的困惑度，选择困惑度最低的
    ...
    for choice in choices:
        ...
        # 计算 context 的长度（用于定位 choice 的起始位置，以计算loss）
        ...
        # 前向传播
        with torch.no_grad():
            # 输入context（文本+问题）+choice，但是后面只计算choice的token的loss
            # 比如输出喜剧片，预测的是喜欢，但是还是输入喜剧去预测片，计算片的loss
